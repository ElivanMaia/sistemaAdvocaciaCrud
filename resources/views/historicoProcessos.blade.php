            ...
            <div class="details">
                <div class="recentOrders">
                    ...
                    @if ($historicoProcessos->isEmpty())
                        <div class="no-data">
                            Nenhum processo excluído registrado.
                        </div>
                    @else
                                ...
                                <tbody>
                                    @foreach ($historicoProcessos as $historico)
                                        <tr>
                                            ...
                                            <td>
                                                <form action="{{ route('processos.restore', ['id' => $historico->id]) }}"
                                                    method="POST" style="display:inline;"
                                                    onsubmit="return confirm('Tem certeza que deseja restaurar este processo?');">
                                                    @csrf
                                                    <button type="submit" class="restorebtn">
                                                        Restaurar
                                                    </button>
                                                </form>

                                                <form id="delete-form-{{ $historico->id }}"
                                                    action="{{ route('processoshistorico.destroy', $historico->id) }}"
                                                    method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="deletebtn"
                                                        onclick="confirmDelete({{ $historico->id }})">
                                                        Excluir
                                                    </button>
                                                </form>
                                            </td>
    ...
    <script>
        function confirmRestore(id) {
            if (confirm('Tem certeza que deseja restaurar este processo?')) {
                window.location.href = "{{ url('processos.restore') }}/" + id;
            }
        }
